...
def getCubeStackTimestamps() {
     ...
     */
    def cubeStackPattern = ~/cube-stack-(?<ts>[0-9]+)-im[01]\.ome\.tiff/

    def timestamps = [] as Set
    cubeAlignPaths.imagesDir.toFile().eachFile(FileType.FILES) {
        def filename = it.toPath().getFileName().toString()
        Matcher match = filename =~ cubeStackPattern
        if (match.matches()) {
            timestamps.add(match.group("ts") as int)
        }
    }
    return timestamps
}
...
def doComputeCubeAlignments(int timestamp) {
     ...
     */
    def csvExists = cubeAlignPaths.scalesShiftsDir.resolve("${timestamp}.csv").toFile().exists()
    def gifExists = cubeAlignPaths.gifsDir.resolve("${timestamp}.gif").toFile().exists()
    def plotExists = cubeAlignPaths.plotsDir.resolve("${timestamp}.svg").toFile().exists()
